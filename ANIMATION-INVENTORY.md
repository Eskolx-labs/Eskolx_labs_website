...
- **STATUS: DONE** (committed 8b2b8e3, cue reworked 16b858e). One timeline now: the mark settles (power3.out, scale 0.98), the seal stamps onto it (power4.in), the paper gives a hair (1.03) and settles — the follow-through the motto taught. A gold hairline draws beneath as the seal stamps (the book's rule language opening the cover). The hand-off recedes: scale 0.97, -1.2° turn as the field turns to day. The scroll cue is an invitation — the text plus a bordered chevron that bounces gently (2.2s ease-in-out, compositor-only, under the reduced-motion kill switch), visible on phones and every window size, yielding to the next spread as the reader commits. Verified beat-by-beat, 70/70 regression checks.
...
- **STATUS: DONE** (committed 947349e, room reworked 16b858e). The stack travel stays linear on purpose (the measured grammar — stake lighting, jumps, aria-current — derives from the linear translate). The arrivals now live in the content: each plate's heading settles as its plate frames (back.out(1.4), measured fractions 0/1/3/2/3/1 stable across viewports; plate 1 settled at the open). The room is 300vh so the stack finishes at ~69% of it — the fourth tier holds its reading window through the whole tail, and the stage lifts away from finish+5% to 97% (a hair of y with the fade, the book's exit grammar) into the dawn seam. On phones, where the stack finishes past the room's end, there is no dead tail to fill, so the stage stays with tier 4 in the window. The desktop current stake's wine pop was dead (aria-current rule sat before data-lit, lost the cascade) — moved after, now pops with translateY -1 + scale 1.04; phone strip keeps its 1.15 pop. 65/65 regression checks.
...
- **STATUS: DONE** (committed 0df33e7, pen reworked 16b858e). The hand now carries a pen: a wine ink bar blinks while the hand writes (its cycle covers exactly the writing window — compressing for short titles so the last blink ends as the last character lands — and lifts with a quick 0.1s fade, reversible — no kill), and a second pen rides the body's words and lifts at the paragraph's end. The icon draws with a pen-stroke ease (power2.in: fast start, slow finish). The plate lifts away with a slight turn (-1.2°) — the hand-off register. Verified: the pen is bright through the title (one blink), hands off at the title's end, the body pen rides all 40 words and lifts at the end, no resurrection; reduced motion hides the cursor and shows static text; phone flows with the typing engine intact; 65/65 regression checks.
...
- **STATUS: DONE** (no code change — already expert-grade). The press is the dawn seam's authored moment: the bite rises (power2.in), settles (power2.out), releases; the rest of the grain stays, so the press reads as real. The dusk rides the lerp on purpose (text and background move together there, so no press is wanted).
