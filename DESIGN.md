...
- **Living grain** (`components/living-grain.tsx`): ~~fixed canvas of fibers blown by Lenis velocity; desktop, motion-permitting only~~. **REMOVED** — the dust layer was imperceptible at its honest alpha and cost a canvas paint every frame; the page reads cleaner without it.
