...
- **The cover is night; the identity sits in day**: the hero holds `LOAM` while the cover reads — the wordmark docks, the motto performs (DeepLearning rises, the "ing" peels, the words trade into "Learn deep", "build expertise." settles), the mission lines rise — then turns to `PARCHMENT` across its content-empty tail (`turnAt: [0.9, 1]`, after the copy has lifted away). The book opens dark, and the page-turn runs over nothing. The identity chapter keeps `PARCHMENT → PARCHMENT`; a short `h-[55vh]` dusk seam (28vh below md) carries `PARCHMENT → LOAM` into the trellis. Nav chrome tints with the field vars even ungrounded, so it reads cream over the dark cover and ink over paper.
