...
- **Eshcol identities, typed by hand** (`components/origin-story.tsx`): one pinned spread; each plate rises alone — icon draws, kicker stamps, then title (per character) and body (per word) write themselves inside tiny per-unit windows, so scroll speed literally controls writing speed. Plates hold for roughly a third of a 340vh room and lift away read.
