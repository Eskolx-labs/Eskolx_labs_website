...
**The Mobile Pin Rule.** By default pins collapse below `md`: the spacer drops to auto and the sticky shell becomes flow, and the Root builds no scrubbed timeline. The **mobile pin tier** (`pinMobile` + `mobilePins`) — the four-tier trellis (240vh), the guide's bar (220vh), and the seal flood (180vh) — keeps shortened rooms on portrait phones AND on short desktop windows (500–699px of height): their plate stacks and centered seal fit those shells, so the animations never disappear there. A two-step height guard: under 500px of viewport height *everything* collapses to flow (a 100vh shell clips any room), and on 500–699px heights every non-tier pin collapses while flow chapters keep their viewport-crossing timelines (a crossing scrub needs no shell height). The hero's copy block stacks statically below `min-height: 900px` (its absolute bottom-anchored composition needs the tall shell, where the mark and copy never collide), and the nav logo simply stays visible instead of docking. Mobile waypoints light exactly like desktop now that the tier rooms build timelines there. The seal flood's pinned scrub choreographs every size — the old below-md one-shot was deleted (it double-drove the seal: its final opacity:0 killed the element the scrub was still animating, leaving phones a blank room for ~90vh). Plate windows derive from the plate itself — the shell reserve is a ceiling, never the target — and use overflow:clip so touch devices cannot pan the boxes instead of the page. Pinned-room headings are static furniture: a viewport Reveal measured inside a sticky shell bakes in the stuck position and scrambles on resize. Flow chapters build their timelines on phones too (a crossing scrub needs no shell height).
...
- **The cover is night; the identity sits in day**: the hero holds `LOAM` while the cover reads — headline, mission, CTAs — then turns to `PARCHMENT` across its content-empty tail (`turnAt: [0.84, 1]`, after the copy has lifted away). The book opens dark, and the page-turn runs over nothing. The identity chapter keeps `PARCHMENT → PARCHMENT`; a short `h-[55vh]` dusk seam (28vh below md) carries `PARCHMENT → LOAM` into the trellis. Nav chrome tints with the field vars even ungrounded, so it reads cream over the dark cover and ink over paper.
...
- **Mobile gets its cover art**: the hero joins the mobile pin tier too (200vh room) — phones get the full cover lift, night holding while the headline rises and the field turning across the room instead of flickering to mud in 170px, and a pinned shell that can never scroll the wordmark under the nav. The seal flood scrubs at every size (pinned at 180vh; the seal floods to scale 16 and releases past 19), typing runs at every width, and the header always offers Telegram; the GitHub pill joins from `sm`.
